        ...
        static void Main()
        {
            ...
            for (int i = 1; i <= strings.Count - 2; i++)
            {
                for (int j = 1; j <= jagged[i].Length - 1; j++)
                {
                    ...
                    if (string.Equals(jagged[i][j].ToString(), jagged[i - 1][j - 1].ToString(), StringComparison.OrdinalIgnoreCase) &&
                       string.Equals(jagged[i - 1][j - 1].ToString(), jagged[i - 1][j + 1].ToString(), StringComparison.OrdinalIgnoreCase) &&
                       string.Equals(jagged[i - 1][j + 1].ToString(), jagged[i + 1][j - 1].ToString(), StringComparison.OrdinalIgnoreCase) &&
                       string.Equals(jagged[i + 1][j - 1].ToString(), jagged[i + 1][j + 1].ToString(), StringComparison.OrdinalIgnoreCase))
                    {
            ...
            while (count > 0)
            {
                jagged[pntsToRemove[count - 1].X][pntsToRemove[count - 1].Y] =
                    jagged[pntsToRemove[count - 1].X - 1][pntsToRemove[count - 1].Y - 1] =
                    jagged[pntsToRemove[count - 1].X + 1][pntsToRemove[count - 1].Y - 1] =
                    jagged[pntsToRemove[count - 1].X + 1][pntsToRemove[count - 1].Y + 1] =
                    jagged[pntsToRemove[count - 1].X - 1][pntsToRemove[count - 1].Y + 1] = ' ';
                count--;
